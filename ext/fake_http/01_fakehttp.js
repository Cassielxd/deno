...
/** @type {(self: FakeHttpConn, rid: number) => boolean} */
...
class FakeHttpConn {
    ...
    async nextRequest() {
        let nextRequest;
        try {
            nextRequest = await op_fake_http_accept(this.#rid);
        } catch (error) {
            this.close();
        ...
        const innerRequest = newInnerRequest(
            method,
            url,
            () => op_fake_http_headers(readStreamRid),
...
function createRespondWith(
    ...
    return async function respondWith(resp) {
        try {
            ...
            try {
                await op_fake_http_write_headers(
                    writeStreamRid,
            ...
            if (isStreamingResponseBody) {
                ...
                if (resourceBacking) {
                    ...
                    try {
                        await op_fake_http_write_resource(
                            writeStreamRid,
                            resourceBacking.rid
                        );
                    ...
                    while (true) {
                        ...
                        try {
                            await op_fake_http_write(writeStreamRid, value);
                        } catch (error) {
                            const connError = httpConn[connErrorSymbol];
                            if (
                                ObjectPrototypeIsPrototypeOf(BadResourcePrototype, error) &&
                ...
                if (success) {
                    try {
                        await op_fake_http_shutdown(writeStreamRid);
                    } catch (error) {
                        await reader.cancel(error);
                        throw error;
                    }
